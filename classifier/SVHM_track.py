...
Modified: All outputs saved to SVHN_10k folder structure
...
def train_flow_matching(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            os.makedirs('SVHN_10k/checkpoints', exist_ok=True)
            checkpoint = {
                'epoch': epoch,
            ...
            torch.save(checkpoint, f'SVHN_10k/checkpoints/flow_epoch_{epoch+1}.pt')
...
def train_flow_classifier(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            os.makedirs('SVHN_10k/checkpoints', exist_ok=True)
            torch.save(classifier.state_dict(), f'SVHN_10k/checkpoints/classifier_epoch_{epoch+1}.pt')
    
    # Save training history
    os.makedirs('SVHN_10k/training_logs', exist_ok=True)
    np.save('SVHN_10k/training_logs/classifier_losses.npy', np.array(losses))
    np.save('SVHN_10k/training_logs/classifier_accuracies.npy', np.array(accuracies))
    ...
    plt.tight_layout()
    plt.savefig('SVHN_10k/training_logs/classifier_training_curves.png', dpi=150)
    plt.close()
...
def evaluate_classifier_performance(classifier, flow_model, test_loader, device='cuda'):
    """
    Comprehensive Classifier Performance Evaluation
    ...
    print("\n" + "="*80)
    print("COMPREHENSIVE CLASSIFIER PERFORMANCE EVALUATION")
    print("="*80)
    ...
    for i, class_name in enumerate(svhn_classes):
    ...
    # ========================================================================
    os.makedirs('SVHN_10k/evaluation', exist_ok=True)
    
    with open('SVHN_10k/evaluation/classifier_comprehensive_evaluation.txt', 'w') as f:
        f.write("COMPREHENSIVE CLASSIFIER PERFORMANCE EVALUATION\n")
        f.write("="*70 + "\n\n")
        ...
        for i, class_name in enumerate(svhn_classes):
            class_acc = class_correct[i] / class_total[i] if class_total[i] > 0 else 0
            f.write(f"      Digit {class_name}: {class_acc:.4f}\n")
    ...
    plt.tight_layout()
    plt.savefig('SVHN_10k/evaluation/classifier_per_class_accuracy.png', dpi=150)
    plt.close()
    ...
    plt.ylim([0, 1])
    plt.tight_layout()
    plt.savefig('SVHN_10k/evaluation/classifier_accuracy_vs_timestep.png', dpi=150)
    plt.close()
    
    return {
        'clean_accuracy': accuracy_clean,
        'per_class_accuracy': {svhn_classes[i]: class_correct[i] / class_total[i] 
                               for i in range(10)},
...
def sample_flow_ode(model, num_samples, num_steps=50, batch_size=50, use_heun=True, device='cuda'):
    ...
    os.makedirs('SVHN_10k/generated_samples', exist_ok=True)
    ...
    while samples_generated < num_samples:
        ...
        for i in range(curr_batch_size):
            ...
            save_image(x_clamped[i], f'SVHN_10k/generated_samples/sample_{sample_idx:04d}.png')
...
def visualize_samples(model, classifier, num_samples=64):
    ...
    os.makedirs('SVHN_10k/samples', exist_ok=True)
    ...
    save_image(grid, 'SVHN_10k/samples/unconditional_samples.png')
    ...
    for guidance_scale in guidance_pbar:
        ...
        save_image(grid, f'SVHN_10k/samples/guided_scale_{guidance_scale}.png')
    ...
    for class_idx in class_pbar:
        ...
        save_image(grid, f'SVHN_10k/samples/digit_{class_idx}_conditional.png')
    
    class_pbar.close()
    print("All samples saved to 'SVHN_10k/samples/' directory")
...
def main():
    print("="*80)
    print("SVHN Flow Matching with Classifier Guidance")
    ...
    # Create main SVHN_10k directory
    os.makedirs('SVHN_10k', exist_ok=True)
    ...
    if os.path.exists('SVHN_10k/checkpoints/flow_final.pt'):
        print("Loading pre-trained flow model...")
        checkpoint = torch.load('SVHN_10k/checkpoints/flow_final.pt', map_location=device)
        ...
        }, 'SVHN_10k/checkpoints/flow_final.pt')
    ...
    if os.path.exists('SVHN_10k/checkpoints/classifier_final.pt'):
        print("Loading pre-trained classifier...")
        classifier.load_state_dict(torch.load('SVHN_10k/checkpoints/classifier_final.pt', map_location=device))
    else:
        ...
        torch.save(classifier.state_dict(), 'SVHN_10k/checkpoints/classifier_final.pt')
    ...
    print("\nSaving results...")
    with open('SVHN_10k/comprehensive_results.txt', 'w') as f:
        f.write("SVHN Classifier Guidance - Comprehensive Results\n")
        f.write("="*70 + "\n\n")
    ...
    print("COMPLETE!")
    print("="*80)
    print(f"Results saved to 'SVHN_10k/comprehensive_results.txt'")
    print(f"Visualizations saved to 'SVHN_10k/samples/' directory")
    print(f"Model checkpoints saved to 'SVHN_10k/checkpoints/' directory")
    print(f"Generated samples saved to 'SVHN_10k/generated_samples/' directory")
    print(f"Training logs saved to 'SVHN_10k/training_logs/' directory")
    print(f"Evaluation metrics saved to 'SVHN_10k/evaluation/' directory")
