...
ADAPTED FOR CIFAR-10 DATASET
Modified: All outputs saved to CIFAR10_10k folder structure
FIXED: Improved classifier guidance with proper normalization and tracking
"""
...
def train_flow_matching(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            os.makedirs('CIFAR10_10k/checkpoints', exist_ok=True)
            checkpoint = {
                'epoch': epoch,
            ...
            torch.save(checkpoint, f'CIFAR10_10k/checkpoints/flow_epoch_{epoch+1}.pt')
...
def train_flow_classifier(
    ...
    """Train classifier on flow-matched interpolated data with tracking"""
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            os.makedirs('CIFAR10_10k/checkpoints', exist_ok=True)
            torch.save(classifier.state_dict(), f'CIFAR10_10k/checkpoints/classifier_epoch_{epoch+1}.pt')
    
    # Save training history
    os.makedirs('CIFAR10_10k/training_logs', exist_ok=True)
    np.save('CIFAR10_10k/training_logs/classifier_losses.npy', np.array(losses))
    np.save('CIFAR10_10k/training_logs/classifier_accuracies.npy', np.array(accuracies))
    ...
    plt.tight_layout()
    plt.savefig('CIFAR10_10k/training_logs/classifier_training_curves.png', dpi=150)
    plt.close()
...
def evaluate_classifier_performance(classifier, flow_model, test_loader, device='cuda'):
    """
    Comprehensive Classifier Performance Evaluation for CIFAR-10
    Evaluates classifier on:
    1. Clean test images (t=1.0)
    2. Flow-interpolated images at various timesteps
    3. Per-class accuracy breakdown
    4. Pure noise (sanity check)
    """
    ...
    print("\n" + "="*80)
    print("COMPREHENSIVE CLASSIFIER PERFORMANCE EVALUATION - CIFAR-10")
    print("="*80)
    
    # ========================================================================
    # 1. TEST ON CLEAN IMAGES (t=1.0)
    # ========================================================================
    ...
    for i, class_name in enumerate(cifar10_classes):
        ...
        print(f"  {class_name:12s}: {class_acc:.4f} ({class_correct[i]}/{class_total[i]})")
    
    # ========================================================================
    # 2. TEST ON FLOW-INTERPOLATED IMAGES AT VARIOUS TIMESTEPS
    # ========================================================================
    ...
    # ========================================================================
    # 3. TEST WITH PURE NOISE (t=0.0) - SANITY CHECK
    # ========================================================================
    ...
    # ========================================================================
    os.makedirs('CIFAR10_10k/evaluation', exist_ok=True)
    
    with open('CIFAR10_10k/evaluation/classifier_comprehensive_evaluation.txt', 'w') as f:
        f.write("COMPREHENSIVE CLASSIFIER PERFORMANCE EVALUATION - CIFAR-10\n")
        f.write("="*70 + "\n\n")
        
        f.write("1. CLEAN IMAGES (t=1.0):\n")
        f.write(f"   Overall Accuracy: {accuracy_clean:.4f}\n\n")
        f.write("   Per-Class Accuracy:\n")
        for i, class_name in enumerate(cifar10_classes):
            class_acc = class_correct[i] / class_total[i] if class_total[i] > 0 else 0
            f.write(f"      {class_name:12s}: {class_acc:.4f}\n")
        
        f.write("\n2. FLOW-INTERPOLATED IMAGES:\n")
        for t_val, acc in timestep_accuracies:
            f.write(f"   t={t_val:.1f}: {acc:.4f}\n")
        
        f.write(f"\n3. PURE NOISE (t=0.0):\n")
        f.write(f"   Accuracy: {accuracy_noise:.4f}\n")
        f.write(f"   Expected: ~0.1000\n")
        f.write(f"   Difference: {abs(accuracy_noise - 0.1):.4f}\n")
    
    # Plot per-class accuracies
    plt.figure(figsize=(12, 6))
    plt.bar(range(10), [class_correct[i] / class_total[i] if class_total[i] > 0 else 0 
                        for i in range(10)])
    plt.xlabel('Class')
    plt.ylabel('Accuracy')
    plt.title(f'Classifier Per-Class Accuracy on Clean Images (Overall: {accuracy_clean:.4f})')
    plt.xticks(range(10), cifar10_classes, rotation=45, ha='right')
    plt.ylim([0, 1])
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('CIFAR10_10k/evaluation/classifier_per_class_accuracy.png', dpi=150)
    plt.close()
    
    # Plot accuracy vs timestep
    plt.figure(figsize=(10, 6))
    t_values = [t for t, _ in timestep_accuracies]
    acc_values = [acc for _, acc in timestep_accuracies]
    plt.plot(t_values, acc_values, marker='o', linewidth=2, markersize=8)
    plt.xlabel('Timestep (t)')
    plt.ylabel('Accuracy')
    plt.title('Classifier Accuracy vs Flow Timestep')
    plt.grid(True, alpha=0.3)
    plt.xlim([0, 1])
    plt.ylim([0, 1])
    plt.tight_layout()
    plt.savefig('CIFAR10_10k/evaluation/classifier_accuracy_vs_timestep.png', dpi=150)
    plt.close()
...
# Keep backward compatibility by aliasing the old function name
def evaluate_classifier(classifier, flow_model, test_loader, device='cuda'):
    """Backward compatibility wrapper for evaluate_classifier_performance"""
    return evaluate_classifier_performance(classifier, flow_model, test_loader, device)

# ============================================================================
# SAMPLING FUNCTIONS - FIXED CLASSIFIER GUIDANCE
# ============================================================================

@torch.no_grad()
def sample_flow_ode(model, num_samples, num_steps=50, batch_size=50, use_heun=True, device='cuda'):
    ...
    os.makedirs('CIFAR10_10k/generated_samples', exist_ok=True)
    ...
    while samples_generated < num_samples:
        ...
        for i in range(curr_batch_size):
            ...
            save_image(x_clamped[i], f'CIFAR10_10k/generated_samples/sample_{sample_idx:04d}.png')
...
def sample_with_classifier_guidance(
    ...
    """
    FIXED: Sample using classifier guidance for flow matching
    
    Properly implements: v_guided = v_uncond + guidance_scale * normalized_gradient
    where gradient is ∇_x log p(y|x_t) scaled to match velocity magnitude
    """
    ...
    for step in step_pbar:
        ...
        # Get unconditional velocity (no gradient tracking)
        with torch.no_grad(), amp.autocast():
            v_uncond = sample_fn(x, t_tensor)
        
        # Compute classifier gradient: ∇_x log p(y|x_t)
        x_input = x.detach().requires_grad_(True)
        with amp.autocast():
            logits = classifier(x_input, t_tensor)
            log_probs = F.log_softmax(logits, dim=-1)
            selected = log_probs[range(num_samples), class_labels]
        
        # Get gradient
        grad_log_py = torch.autograd.grad(selected.sum(), x_input)[0]
        
        # CRITICAL FIX: Normalize gradient per-sample to match velocity magnitude
        # This ensures guidance has consistent effect regardless of gradient scale
        v_norm = v_uncond.view(num_samples, -1).norm(dim=1, keepdim=True).view(-1, 1, 1, 1)
        grad_norm = grad_log_py.view(num_samples, -1).norm(dim=1, keepdim=True).view(-1, 1, 1, 1)
        grad_normalized = grad_log_py * (v_norm / (grad_norm + 1e-8))
        
        # Apply guidance: v_guided = v_uncond + guidance_scale * normalized_gradient
        v_guided = v_uncond + guidance_scale * grad_normalized
        
        if use_heun and step < num_steps - 1:
            # Heun's method (2nd order Runge-Kutta)
            x_euler = x.detach() + (t_next - t) * v_guided
            
            t_next_tensor = torch.full((num_samples,), t_next, device=device)
            
            # Get next unconditional velocity
            with torch.no_grad(), amp.autocast():
                v_next_uncond = sample_fn(x_euler, t_next_tensor)
            
            # Compute next gradient
            x_euler_input = x_euler.detach().requires_grad_(True)
            with amp.autocast():
                logits_next = classifier(x_euler_input, t_next_tensor)
                log_probs_next = F.log_softmax(logits_next, dim=-1)
                selected_next = log_probs_next[range(num_samples), class_labels]
            
            grad_log_py_next = torch.autograd.grad(selected_next.sum(), x_euler_input)[0]
            
            # Normalize next gradient
            v_next_norm = v_next_uncond.view(num_samples, -1).norm(dim=1, keepdim=True).view(-1, 1, 1, 1)
            grad_next_norm = grad_log_py_next.view(num_samples, -1).norm(dim=1, keepdim=True).view(-1, 1, 1, 1)
            grad_next_normalized = grad_log_py_next * (v_next_norm / (grad_next_norm + 1e-8))
            
            v_next_guided = v_next_uncond + guidance_scale * grad_next_normalized
            
            # Average the two velocities (Heun's method)
            x = x.detach() + (t_next - t) * 0.5 * (v_guided + v_next_guided)
        else:
            # Euler's method
            x = x.detach() + (t_next - t) * v_guided
        
        step_pbar.set_postfix({
            't': f'{t:.3f}',
            'v_mag': f'{v_uncond.abs().mean():.3f}',
            'g_mag': f'{grad_normalized.abs().mean():.3f}'
        })
...
def evaluate_model_comprehensive(
    ...
    """Comprehensive evaluation with FID and Precision-Recall - MEMORY OPTIMIZED"""
    ...
    uncond_samples = sample_flow_ode(flow_model, num_samples, num_steps=50, batch_size=50, use_heun=True)
...
def visualize_samples(model, classifier, num_samples=64):
    ...
    os.makedirs('CIFAR10_10k/samples', exist_ok=True)
    
    print("\nGenerating visualization samples...")
    
    print("Generating unconditional samples...")
    uncond_samples = sample_flow_ode(model, num_samples, num_steps=50, batch_size=50, use_heun=True)
    grid = make_grid(uncond_samples, nrow=8, normalize=True, value_range=(-1, 1))
    save_image(grid, 'CIFAR10_10k/samples/unconditional_samples.png')
    ...
    for guidance_scale in guidance_pbar:
        ...
        save_image(grid, f'CIFAR10_10k/samples/guided_scale_{guidance_scale}.png')
    ...
    for class_idx in class_pbar:
        class_pbar.set_description(f"Generating {class_names[class_idx]}")
        ...
        save_image(grid, f'CIFAR10_10k/samples/{class_names[class_idx]}_conditional.png')
    
    class_pbar.close()
    print("All samples saved to 'CIFAR10_10k/samples/' directory")
...
def main():
    ...
    print("FIXED: Proper classifier guidance normalization")
    print("="*80)
    ...
    # Create main CIFAR10_10k directory
    os.makedirs('CIFAR10_10k', exist_ok=True)
    ...
    if os.path.exists('CIFAR10_10k/checkpoints/flow_final.pt'):
        print("Loading pre-trained flow model...")
        checkpoint = torch.load('CIFAR10_10k/checkpoints/flow_final.pt', map_location=device)
        ...
        }, 'CIFAR10_10k/checkpoints/flow_final.pt')
    ...
    if os.path.exists('CIFAR10_10k/checkpoints/classifier_final.pt'):
        print("Loading pre-trained classifier...")
        classifier.load_state_dict(torch.load('CIFAR10_10k/checkpoints/classifier_final.pt', map_location=device))
    else:
        ...
        torch.save(classifier.state_dict(), 'CIFAR10_10k/checkpoints/classifier_final.pt')
    
    # Evaluate classifier
    print("\n" + "="*80)
    print("CLASSIFIER EVALUATION")
    print("="*80)
    evaluate_classifier(classifier, flow_model, test_loader)
    ...
    print("\nSaving results...")
    with open('CIFAR10_10k/comprehensive_results.txt', 'w') as f:
    ...
    print("COMPLETE!")
    print("="*80)
    print(f"Results saved to 'CIFAR10_10k/comprehensive_results.txt'")
    print(f"Visualizations saved to 'CIFAR10_10k/samples/' directory")
    print(f"Model checkpoints saved to 'CIFAR10_10k/checkpoints/' directory")
    print(f"Generated samples saved to 'CIFAR10_10k/generated_samples/' directory")
    print(f"Training logs saved to 'CIFAR10_10k/training_logs/' directory")
    print(f"Evaluation metrics saved to 'CIFAR10_10k/evaluation/' directory")
