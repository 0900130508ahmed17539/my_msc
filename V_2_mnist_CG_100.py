...
class ImprovedMNISTFlowClassifier(nn.Module):
    """
    MUCH MORE COMPLEX time-conditioned classifier for MNIST flow matching guidance.
    ...
    def __init__(
        self,
        num_classes=10,
        time_emb_dim=256,  # Increased significantly
        base_channels=128,  # Doubled
        dropout=0.25,
        num_res_blocks=3  # Multiple residual blocks per stage
    ):
        super().__init__()
        ...
        self.conv_in = nn.Sequential(
            conv_nd(2, 1, base_channels, 7, padding=3),
            normalization(base_channels),
            SiLU()
        )
        
        # Stage 1: 28x28 -> 28x28 (base_channels)
        ...
        # Stage 2: 28x28 -> 14x14 (base_channels * 2)
        ...
        # Stage 3: 14x14 -> 7x7 (base_channels * 4)
        ...
        # Stage 4: 7x7 -> 4x4 (base_channels * 8)
...
def train_flow_classifier(
    ...
    flow_model,
    dataloader,
    num_epochs=50,  # Increased significantly for complex model
    lr=5e-4,  # Reduced initial LR for stability
    warmup_epochs=5,
    device='cuda'
):
...
def main():
    ...
    PHYSICAL_BATCH_SIZE = 32
    NUM_EPOCHS_FLOW = 50
    NUM_EPOCHS_CLASSIFIER = 50  # Increased for complex model
    ...
    classifier = ImprovedMNISTFlowClassifier(
        num_classes=10, 
        time_emb_dim=256,  # Much larger
        base_channels=128,  # Doubled
        dropout=0.25,  # Increased
        num_res_blocks=3  # 3 residual blocks per stage
    ).to(device)
    
    print(f"Classifier parameters: {sum(p.numel() for p in classifier.parameters()):,}")
    
    # Use a different checkpoint name for the new architecture
    checkpoint_path = 'mnist_classifier/checkpoints/deep_classifier_v2_final.pt'
    
    if os.path.exists(checkpoint_path):
        print("Loading pre-trained DEEP classifier v2...")
        classifier.load_state_dict(torch.load(checkpoint_path, map_location=device))
    else:
        print(f"\nTraining MUCH DEEPER classifier for {NUM_EPOCHS_CLASSIFIER} epochs...")
        ...
        print(f"Saved new classifier to {checkpoint_path}")
