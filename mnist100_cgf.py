...
# ============================================================================
# PRECISION-RECALL METRICS (Adapted for MNIST)
# ============================================================================

Manifold = namedtuple("Manifold", ["features", "radii"])

class EfficientPrecisionRecall:
    """Memory and compute efficient Precision-Recall evaluator for MNIST"""
    
    def __init__(self, k=3, device="cuda"):
        self.k = k
        self.device = device
        
        # Load VGG16 (we'll still use it but convert grayscale to RGB)
    ...
    def extract_features_fast(self, dataloader, max_samples=None):
        """Extract features with optimizations for MNIST"""
        features_list = []
        samples_processed = 0
...
# ============================================================================
# FID COMPUTATION FOR MNIST
# ============================================================================

class InceptionFeatureExtractorMNIST:
    """Extract features using pretrained InceptionV3 for MNIST"""
    ...
    def extract_features(self, images, batch_size=16):
        """Extract InceptionV3 features from MNIST images"""
        if images.dim() == 3:
...
# ============================================================================
# U-NET COMPONENTS (Adapted for MNIST)
# ============================================================================
...
# ============================================================================
# IMPROVED CLASSIFIER FOR MNIST - MUCH DEEPER AND MORE COMPLEX
# ============================================================================

class ImprovedMNISTFlowClassifier(nn.Module):
    """
    SIGNIFICANTLY IMPROVED time-conditioned classifier for MNIST flow matching guidance.
    ...
    def __init__(
        self,
        num_classes=10,
        time_emb_dim=128,  # Increased from 64
        base_channels=64,  # Increased from 32
        dropout=0.2  # Increased from 0.1
    ):
        super().__init__()
        
        # Enhanced time embedding with more layers
        self.time_embed = nn.Sequential(
        ...
        # More complex backbone with residual blocks
        # Block 1: 28x28 -> 28x28
        self.conv1 = nn.Sequential(
            conv_nd(2, 1, base_channels, 3, padding=1),
            normalization(base_channels),
            SiLU(),
            conv_nd(2, base_channels, base_channels, 3, padding=1),
            normalization(base_channels),
            SiLU(),
        )
        ...
        self.conv2 = nn.Sequential(
            conv_nd(2, base_channels, base_channels * 2, 3, stride=2, padding=1),
            normalization(base_channels * 2),
            SiLU(),
            conv_nd(2, base_channels * 2, base_channels * 2, 3, padding=1),
            normalization(base_channels * 2),
            SiLU(),
        )
        ...
        self.conv3 = nn.Sequential(
            conv_nd(2, base_channels * 2, base_channels * 4, 3, stride=2, padding=1),
            normalization(base_channels * 4),
            SiLU(),
            conv_nd(2, base_channels * 4, base_channels * 4, 3, padding=1),
            normalization(base_channels * 4),
            SiLU(),
        )
        ...
        self.conv4 = nn.Sequential(
            conv_nd(2, base_channels * 4, base_channels * 8, 3, stride=2, padding=1),
            normalization(base_channels * 8),
            SiLU(),
            conv_nd(2, base_channels * 8, base_channels * 8, 3, padding=1),
            normalization(base_channels * 8),
            SiLU(),
        )
        
        # Simple attention mechanism
        self.attention = AttentionBlock(base_channels * 8, num_heads=4, num_head_channels=64)
        
        self.pool = nn.AdaptiveAvgPool2d((1, 1))
        
        # Much deeper classifier head
        self.classifier = nn.Sequential(
            linear(base_channels * 8 + time_emb_dim, 512),
            SiLU(),
            nn.Dropout(dropout),
            linear(512, 256),
            SiLU(),
...
def train_flow_matching(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            os.makedirs('mnist_classifier/checkpoints', exist_ok=True)
            checkpoint = {
                'epoch': epoch,
            ...
            torch.save(checkpoint, f'mnist_classifier/checkpoints/flow_epoch_{epoch+1}.pt')
...
def train_flow_classifier(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            os.makedirs('mnist_classifier/checkpoints', exist_ok=True)
            torch.save(classifier.state_dict(), f'mnist_classifier/checkpoints/classifier_epoch_{epoch+1}.pt')
...
def sample_flow_ode(model, num_samples, num_steps=50, batch_size=50, use_heun=True, device='cuda', save_individual=False):
    ...
    if save_individual:
        os.makedirs('mnist_classifier/generated_samples', exist_ok=True)
    ...
    while samples_generated < num_samples:
        ...
        if save_individual:
            ...
            for i in range(curr_batch_size):
                ...
                save_image(x_clamped[i], f'mnist_classifier/generated_samples/sample_{sample_idx:04d}.png')
...
def evaluate_model_comprehensive(
    ...
    """Comprehensive evaluation with FID and Precision-Recall for MNIST"""
    
    print(f"\n{'='*70}")
    print(f"Comprehensive Model Evaluation - MNIST ({num_samples} samples)")
    print(f"{'='*70}")
    ...
    feature_extractor = InceptionFeatureExtractorMNIST(device)
...
def visualize_samples(model, classifier, num_samples=64):
    ...
    os.makedirs('mnist_classifier/samples', exist_ok=True)
    ...
    save_image(grid, 'mnist_classifier/samples/unconditional_samples.png')
    ...
    for guidance_scale in guidance_pbar:
        ...
        save_image(grid, f'mnist_classifier/samples/guided_scale_{guidance_scale}.png')
    
    guidance_pbar.close()
    
    print("\nGenerating class-conditional samples...")
    class_pbar = tqdm(enumerate(mnist_classes), total=len(mnist_classes))
    
    for class_idx, class_name in class_pbar:
        class_pbar.set_description(f"Generating digit {class_name}")
        ...
        save_image(grid, f'mnist_classifier/samples/digit_{class_name}_conditional.png')
    
    class_pbar.close()
    print("All samples saved to 'mnist_classifier/samples/' directory")
...
def main():
    print("="*80)
    print("MNIST Flow Matching with IMPROVED Classifier Guidance")
    ...
    PHYSICAL_BATCH_SIZE = 32
    NUM_EPOCHS_FLOW = 50
    NUM_EPOCHS_CLASSIFIER = 30  # Increased from 20
    ...
    if os.path.exists('mnist_classifier/checkpoints/flow_final.pt'):
        print("Loading pre-trained flow model...")
        checkpoint = torch.load('mnist_classifier/checkpoints/flow_final.pt', map_location=device)
        ...
        }, 'mnist_classifier/checkpoints/flow_final.pt')
    
    # Initialize IMPROVED classifier
    print("\nInitializing IMPROVED classifier...")
    classifier = ImprovedMNISTFlowClassifier(
        num_classes=10, 
        time_emb_dim=128,  # Increased
        base_channels=64,  # Increased
        dropout=0.2  # Increased
    ).to(device)
    
    print(f"Classifier parameters: {sum(p.numel() for p in classifier.parameters()):,}")
    
    if os.path.exists('mnist_classifier/checkpoints/improved_classifier_final.pt'):
        print("Loading pre-trained IMPROVED classifier...")
        classifier.load_state_dict(torch.load('mnist_classifier/checkpoints/improved_classifier_final.pt', map_location=device))
    else:
        print(f"\nTraining IMPROVED classifier for {NUM_EPOCHS_CLASSIFIER} epochs...")
        classifier, _ = train_flow_classifier(
            classifier, flow_model, train_loader, num_epochs=NUM_EPOCHS_CLASSIFIER
        )
        os.makedirs('mnist_classifier/checkpoints', exist_ok=True)
        torch.save(classifier.state_dict(), 'mnist_classifier/checkpoints/improved_classifier_final.pt')
    ...
    print(f"Saved {NUM_SAMPLES_SAVE} individual samples to 'mnist_classifier/generated_samples/'")
    ...
    results = evaluate_model_comprehensive(
        ...
        guidance_scales=[0.0, 1.0, 2.0, 4.0, 6.0],  # Added higher scales
    ...
    print("\nSaving results...")
    os.makedirs('mnist_classifier/results', exist_ok=True)
    with open('mnist_classifier/results/improved_comprehensive_results.txt', 'w') as f:
        f.write(f"MNIST IMPROVED Classifier Guidance - Comprehensive Results\n")
        ...
        for key in sorted([k for k in results.keys() if k.startswith('guided_')],
                         key=lambda x: float(x.split('_')[1])):  # FIXED: removed [0]
            scale = key.split('_')[1]
    ...
    print("COMPLETE!")
    print("="*80)
    print(f"Results saved to 'mnist_classifier/results/improved_comprehensive_results.txt'")
    print(f"Visualizations saved to 'mnist_classifier/samples/' directory")
    print(f"Model checkpoints saved to 'mnist_classifier/checkpoints/' directory")
    print(f"{NUM_SAMPLES_SAVE} individual samples saved to 'mnist_classifier/generated_samples/' directory")
